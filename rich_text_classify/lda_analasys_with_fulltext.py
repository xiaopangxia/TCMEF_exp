...
def load_doc_list(case_type= 'all', denoise=True):
    ...
    if case_type == 'pos_case':
        pos_file_list = os.listdir('./page_fulltext/pos_case/')
        pos_text_list = []
        for file in pos_file_list:
            with open('./page_fulltext/pos_case/'+file, 'r') as in_file:
                text = in_file.read()
        ...
        neg_file_list = os.listdir('./page_fulltext/neg_case/')
        neg_text_list = []
        for file in neg_file_list:
            with open('./page_fulltext/neg_case/'+file, 'r') as in_file:
                text = in_file.read()
...
def svm_classify_with_fulltext_topic_vec(svm_kernel='linear', n_features=500, n_topics=200, denoise=False):
    """
    ...
    with open('./lda_fulltext_classify_result/_%s_%s_%s_%s'%(str(svm_kernel), str(n_features), str(n_topics), str(denoise))+'_train.txt', 'a') as out_file:
        out_file.write(train_socre)

    y_hat = clf.predict(x_test)
    test_score = str(classification_report(y_test, y_hat, target_names=['neg', 'pos_case'], digits=4))
    with open('./lda_fulltext_classify_result/_%s_%s_%s_%s'%(str(svm_kernel), str(n_features), str(n_topics), str(denoise))+'_test.txt', 'a') as out_file:
        out_file.write(test_score)



n_feature_list = [200, 400, 600, 800, 1000]
n_topic_list = [20, 40, 60, 80, 100, 120, 140, 160, 180, 200]
    ...
    for n_topic in n_topic_list:
        for denois in denois_list:
            for svm_kernel in svm_kernel_list:
                svm_classify_with_fulltext_topic_vec(svm_kernel, n_feature, n_topic, denois)
