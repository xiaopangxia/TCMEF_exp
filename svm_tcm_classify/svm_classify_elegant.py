...
class FeaturePrepare():
    ...
    def load_case_set(cls, with_name=False):
        ...
        :return: dict
        """
        case_dict = {'pos_case': [], 'neg': []}
        if with_name:
            with open('../data_prepare/manual_filtered_data/pos_data/pos_case_list_with_name.txt', 'r') as in_file:
                for line in in_file.readlines():
                    case_dict['pos_case'].append(line.strip())
        else:
            with open('../data_prepare/manual_filtered_data/pos_data/pos_case_list_without_name.txt', 'r') as in_file:
                for line in in_file.readlines():
                    case_dict['pos_case'].append(line.strip())
...
class SVMClassify():
    ...
    def svm_classify_core(cls, pos_vec_list, neg_vec_list_large, feature_d, log_file, svm_kernel='linear'):
        ...
        for i in range(1, 24):
            # 逐轮增加负样本数
            neg_vec_list = neg_vec_list_large[0:i*500]
            ...
            # 上面按0.8采样还是有问题,测试集正负样例极不均衡,统一用200个
            neg_set = np.array(neg_vec_list)
            x_neg, y_neg = np.split(neg_set, (feature_d,), axis=1)
            ...
            with open('./classify_result_new/'+log_file+'_train.txt', 'a') as out_file:
                out_file.write(train_socre)

            y_hat = clf.predict(x_test)
            test_score = str(classification_report(y_test, y_hat, target_names=['neg', 'pos_case'], digits=4))
            with open('./classify_result_new/'+log_file+'_test.txt', 'a') as out_file:
