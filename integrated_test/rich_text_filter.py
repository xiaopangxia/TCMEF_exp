...
def load_doc_list(case_type= 'all', denoise=True):
    ...
    if case_type == 'pos_case':
        pos_file_list = os.listdir('./rtf_data_fulltext/pos_case/')
        pos_text_list = []
        for file in pos_file_list:
            with open('./rtf_data_fulltext/pos_case/'+file, 'r') as in_file:
                text = in_file.read()
        ...
        neg_file_list = os.listdir('./rtf_data_fulltext/neg_case/')
        neg_text_list = []
        for file in neg_file_list:
            with open('./rtf_data_fulltext/neg_case/'+file, 'r') as in_file:
                text = in_file.read()
...
def count_vectorizer(case_type= 'all', n_features=100, denoise=True):
    ...
    if os.path.exists('./model_file/tf_%s_%s.model' % (str(n_features), str(int(denoise)))):
        tf_vectorizer = joblib.load('./model_file/tf_%s_%s.model' % (str(n_features), str(int(denoise))))
        tf = tf_vectorizer.transform(doc_list)
    else:
        tf_vectorizer = CountVectorizer(max_df=0.95, min_df=2, max_features=n_features,)
        tf = tf_vectorizer.fit_transform(doc_list)
        joblib.dump(tf_vectorizer, './model_file/tf_%s_%s.model' % (str(n_features), str(int(denoise))))
...
def lda_train(n_topics=30, n_features=100, denoise=True):
    ...
    if os.path.exists('./model_file/lda_%s_%s_%s.model' % (str(n_topics), str(n_features), str(int(denoise)))):
        lda = joblib.load('./model_file/lda_%s_%s_%s.model' % (str(n_topics), str(n_features), str(int(denoise))))
    else:
        ...
        joblib.dump(lda, './model_file/lda_%s_%s_%s.model' % (str(n_topics), str(n_features), str(int(denoise))))
...
def svm_classify_with_fulltext_topic_vec(svm_kernel='linear', n_features=800, n_topics=180, denoise=False):
    """
    ...
    x_neg_train, x_neg_test, y_neg_train, y_neg_test = train_test_split(x_neg, y_neg, random_state=11, test_size=0.2)

    pos_set = np.array(pos_vec_list)
    x_pos, y_pos = np.split(pos_set, (n_topics,), axis=1)
    x_pos_train, x_pos_test, y_pos_train, y_pos_test = train_test_split(x_pos, y_pos, random_state=11, test_size=0.2)
    ...
    with open('./rtf_result/_%s_%s_%s_%s'%(str(svm_kernel), str(n_features), str(n_topics), str(denoise))+'_train.txt', 'a') as out_file:
        out_file.write(train_socre)

    y_hat = clf.predict(x_test)
    test_score = str(classification_report(y_test, y_hat, target_names=['neg', 'pos_case'], digits=4))
    with open('./rtf_result/_%s_%s_%s_%s'%(str(svm_kernel), str(n_features), str(n_topics), str(denoise))+'_test.txt', 'a') as out_file:
        out_file.write(test_score)


svm_classify_with_fulltext_topic_vec(n_features=800, n_topics=180, denoise=True)
